# for history of moves, simply added a new instance variable to the player class
# idea being that the moves a player has made are a property of that given player object
# also, easy to increment within the respective Human and Computer classes
...
class Computer < Player
  ...
  def set_name
    ...
    self.strategy = Strategy.new(name)
  end

  def choose
    self.move = Move.new(strategy.choice)
    self.history << "#{move}"
  end
end
...
class Strategy
  ...
  def choice
    if approach == 'Hal'
      %w[rock spock scissors scissors].sample
    elsif approach == 'R2D2'
      'rock'
    elsif approach == 'Chappie'
      %w[rock spock scissors].sample
    elsif approach == 'Number 5'
      %w[lizard paper scissors].sample
    else
      Move::VALUES.sample
    end
  end
...
class RPSGame
  ...
  def display_score
    ...
    puts "#{human.name}: #{human.history.join(', ')}"
    puts "#{computer.name}: #{computer.history.join(', ')}"
    puts "---"
  end
