...
$(function() {
  (function() {
    var inventory = {
      ...
      cacheTemplate: function() {
        var $i_tmpl = $("#inventory_item").remove();
        this.template = Handlebars.compile($i_tmpl.html()); // have template be the compiled Handlebars template function
      },
      add: function() {
        this.lastId++;
        var item = {
      ...
      newItem: function(e) {
        e.preventDefault();
        var item = this.add();
        var $item = $(this.template({itemId: item.id})); // set $item to the jQuery object that is created from the returned template invocation
